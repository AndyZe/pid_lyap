...
#include "pid_lyap/pid_controller.h"
...
int main(int argc, char **argv)
{
  ...
  ros::Publisher chatter_pub = node.advertise<pid_lyap::controller_msg>(topic_from_controller, 1);
...
void chatterCallback(const pid_lyap::plant_msg& msg)
{
  ...
  error.at(0) = msg.setpoint[0]-msg.x[0]; // Current error goes to slot 0
  ...
  u_msg.u[0] = Kp*filtered_error.at(0)+Ki*error_integral+Kd*filtered_error_deriv.at(0);
...
void check_user_input(int& argc, char** argv)
{
  ...
  if ( (args_out.size()<5 || args_out.size()>19) || (args_out.size()%2 != 1) ) // Wrong # or not an even number of arguments
  {
    ...
    cout<<endl;
    cout<<"Example: rosrun pid_lyap pid_controller 1.1 0.1 0.1 1000"<<endl;
    cout<<endl;
